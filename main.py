...
def render_all_font_HUD():
    ...
    if not player.update_render_player:
        if int(font_size_Died) <= WIDTH//6:
            ...
            screen.blit(render_die, (250, HEIGHT // 3))
    if player.pause:
        pygame.draw.rect(screen, DARK_GREEN, (WIDTH // 2 - 110, HEIGHT // 2 + 20, 750, 70))

        font_pause = pygame.font.Font('fonts/pixel_font.otf', 100)
        text = font_pause.render(f'PAUSE', True, CRIMSON)
        screen.blit(text, (WIDTH // 2 - 110, HEIGHT // 2 - 95))

        font_pause = pygame.font.Font('fonts/pixel_font.otf', 100)
        text = font_pause.render(f'PAUSE', True, RED)
        screen.blit(text, (WIDTH // 2 - 100, HEIGHT // 2 - 100))

        font_pause = pygame.font.Font('fonts/pixel_font.otf', 86)
        text = font_pause.render(f'ВЫЙТИ ИЗ ИГРЫ', True, CRIMSON)
        screen.blit(text, (WIDTH // 2 - 110, HEIGHT // 2 + 5))

        font_pause = pygame.font.Font('fonts/pixel_font.otf', 86)
        text = font_pause.render(f'ВЫЙТИ ИЗ ИГРЫ', True, RED)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_s and player.pause:
                if len(buttons_option) - 1 > select_button_options:
                    select_button_options += 1
                    print(select_button_options)
            elif event.key == pygame.K_w and player.pause:
                if -1 < len(buttons_option) - 1:
                    select_button_options -= 1
                    print(select_button_options)
            elif event.key == pygame.K_e and player.pause:
